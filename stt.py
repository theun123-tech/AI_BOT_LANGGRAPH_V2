...
class FluxSession:
    ...
    def __repr__(self):
        ...
        return (f"<FluxSession {self.participant_name} "
...
class FluxSessionManager:
    ...
    async def route_audio(
        ...
        while len(session.audio_buffer) >= FLUX_CHUNK_SIZE:
            ...
            try:
                ...
                print(f"[FluxManager] {self._tag} ⚠️  Queue put failed for "
    ...
    async def close_session(self, participant_id: str) -> None:
        ...
        print(f"[FluxManager] {self._tag} 🔌 Closed session for "
    ...
    async def _get_or_create_session(
        ...
        async with self._create_lock:
            ...
            session.flux_task = asyncio.create_task(
                self._run_flux_session(session)
            )

            print(f"[FluxManager] {self._tag} 🆕 Created Flux session for "
    ...
    async def _run_flux_session(self, session: FluxSession) -> None:
        ...
        async def _on_transcript(text: str, is_final: bool, _unused=None):
            ...
            try:
                if is_final:
                    ...
                    await self._on_final(speaker, text)
                else:
                    session.last_interim = text
                    await self._on_interim(speaker, text)
            except Exception as e:
                print(f"[FluxManager] {self._tag} ⚠️  Callback error for "
                      f"{speaker}: {e}")
        ...
        while session.is_alive and not self._closing:
            try:
                ...
                print(f"[FluxManager] {self._tag} 🏁 Flux session ended "
                ...
                print(f"[FluxManager] {self._tag} ⚠️  Flux error for {speaker} "
                      f"(attempt {session.consecutive_errors}/3): {e}")

                if session.consecutive_errors >= 3:
                    print(f"[FluxManager] {self._tag} ❌ Giving up on "
